...
namespace VxSdkNet {
    ...
    public:
        ...
        /// </summary>
        /// <value><c>true</c> if transcoded, <c>false</c> if not.</value>
        property bool IsTranscoded {
        public:
            bool get() { return _dataInterface->isTranscoded; }
        }
        ...
        property System::Collections::Generic::List<OverlayTypes>^ Overlays {
        public:
            System::Collections::Generic::List<OverlayTypes>^ get() {
                System::Collections::Generic::List<OverlayTypes>^ mlist = gcnew System::Collections::Generic::List<OverlayTypes>();
                for (int i = 0; i < _dataInterface->overlayTypeSize; i++)
                    mlist->Add((OverlayTypes)_dataInterface->overlayTypes[i]);
